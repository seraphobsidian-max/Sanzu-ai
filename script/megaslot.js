...
// ang read->modify->write kapag sabay-sabay tumaya ang mga user.
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (isNaN(bet) || bet < 50) {
    return api.sendMessage("⚠️ Minimum na taya sa Mega Slot ay 🪙 50 Coins.\nHalimbawa: /megaslot 200", threadID, messageID);
  }
  ...
  await withLock(() => {
    ...
    if (eco[senderID].coins < bet) {
      api.sendMessage(`❌ Kulang ang iyong coins! Meron ka lamang 🪙 ${eco[senderID].coins.toLocaleString()} Coins.\nMag-type ng '/coins daily' para sa libreng barya.`, threadID, messageID);
      return;
    }
